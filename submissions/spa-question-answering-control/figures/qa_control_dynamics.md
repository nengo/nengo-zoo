...
**Middle — memory contents.** Plotted against the two bound pairs the network is shown. `RED * CIRCLE` rises during the first statement, then `BLUE * SQUARE` accumulates during the second; both persist into the question phase thanks to the memory's self-feedback.

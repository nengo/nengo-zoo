...
**Left — "Blue triangle"** (`bind(triangle_SP, blue_SP)`): the peak picks out the blue-triangle landmark in the lower-left of the domain.

**Middle — "All triangles"** (`bind(triangle_SP, blue_SP + orange_SP)`): the peak still sits at the blue triangle, but now extends through the orange triangle in the middle-right — the memory has identified both triangular objects via SP algebra applied at query time.

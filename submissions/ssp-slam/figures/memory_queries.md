...
**Left — "Blue triangle"** (`bind(triangle_SP, blue_SP)`): a peak centred at the blue triangle landmark in the upper-left.

**Middle — "All triangles"** (`bind(triangle_SP, blue_SP + orange_SP)`): a strong peak at the blue triangle, with a secondary peak at the orange triangle in the lower-right — the memory has identified both triangular objects via SP algebra at query time.

**Right — "All blues"** (`bind(triangle_SP + square_SP, blue_SP)`): a single peak at the blue triangle (the only blue object in the scene).

At this CI-sized `ssp_dim = 37`, some smearing of the query response onto the agent's transit corridors between landmarks is expected — the memory associates each landmark SP with the SSPs it observed while learning it, which span a short interval of the path. The higher-fidelity setup in Dumont et al. (2023) produces sharper localisation, but the compositional structure of the queries is already visible here.

...
**Right — phase portrait.** LIP's 2-D trajectory (orange) wanders from the origin (gray dot) toward the input direction (black star at `(−0.5, +0.5)`). The output trajectory (red) covers the same path but only after LIP escapes the dashed threshold circle. The wandering after t ≈ 3 s is the integrator amplifying the injected ensemble noise — chunkier evidence streams in the higher-fidelity setting from the book settle more tightly on the input.
